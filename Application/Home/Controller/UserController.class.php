...
class UserController extends BaseController {
    ...
    public function _initialize() {      
        ...
        if(session('?user'))
        {
        	...
        	$nologin = array(
        			...
        			'verifyHandle','reg','regverify','send_sms_reg_code','identity','check_validate_code',
    ...
    public function index(){
        if(IS_POST){
            $data = I('post.');
            $result = M("Users")->where(array("user_id"=>$data['user_id'],"mobile"=>$data['mobile']))
                                ->setField(array("nickname"=>$data['nickname'],"sex"=>$data['sex'],"birthday"=>$data['birthday']));
            if( !$result ){
    ...
    public function reg(){
        ...
        if(IS_POST){
            ...
            $code = I('post.code','');
            //校验验证码
            $codeVerify = $logic->codeVerify( $phone , $code );
            if( $codeVerify ){
            ...
            //验证码检验
            $data = $logic->regverify( $username , $phone , $password , $email );
            if( $data['status'] != 1 )
                $this->error($data['msg']);
            ...
            $this->success($data['msg'],'http://www.zbzixun.com');
            exit;
        }
    ...
    public function bind_mobile(){
        if(IS_POST){
            ...
            if(empty( $phone ) || empty( $code )){
                $this->error("请输入正确手机号或验证码");
            }
            $Model = D('Users');
            //检测手机号是否用过
            $findMobile = $Model->getUser( $phone );
            if( $findMobile ){
                $this->error('此手机号已经使用过!');
            }
            //校验验证码
            ...
            //绑定
            $result = $Model->bindMobile( $phone , $userInfo['user_id']);
            if( $result ){
                $this->success('绑定手机号成功！',U('User/index'),3);
            }else{
                $this->error('绑定手机码失败，请稍候再试~！');
            }
        }else{
            $this->display();
        }
    ...
    **/
    public function consult(){
        ...
        $model['tittle'] = $_POST['question'];
        $model['user_id'] = session('user.user_id');
        $model['detail'] = $_POST['question-con'];
        $model['createtime'] = time();
        $model['path'] = rtrim($_POST['picUrl'],',');
    ...
    public function payTourHandle(){
        $model['pay_id'] = time();
        $model['user_id'] = session('user.user_id');
        $model['money'] = $_POST['money'];
        $model['pay_order_number'] = date('YmdHis');
        $model['is_status'] = 30;
        $model['pay_time'] = time();
    ...
    *检测用户是否存在未使用打赏log
    **/
    public function payTour(){
        $payType = trim(I('post.payType'));
        if( !$payType == 'pay_tour'){
            exit('400');
        }else{
            $result = M('PayTour')->where(array("user_id"=>session('user.user_id'),"is_status"=>"30","status"=>"10"))
                                  ->order("pay_time desc")
                                  ->select();
            if( $result ){
                exit('200');
    ...
    public function myconsult(){
        ...
        $res = $Model->where("user_id='{$user_id}'")
                     ->field("consult_id,tittle")
                     ->order('createtime desc')
                     ->page("$p,$size")
                     ->select();
        $count = $Model->where( array("user_id"=>$user_id) )->count();// 查询满足要求的总记录数
    ...
    public function myactivity(){
        $p = empty($_REQUEST['p']) ? 1 : $_REQUEST['p'];
        $size = empty($_REQUEST['size']) ? 10 : $_REQUEST['size'];
        $Model = M("activity_entry a");
        $user_id = session('user.user_id');
        $res = $Model->where("a.user_id='{$user_id}' AND a.is_del='1'")
                     ->field("a.activity_id,is_status,b.title")
                     ->join("tp_activity b ON a.activity_id=b.activity_id")
                     ->order('a.entry_id desc')
                     ->page("$p,$size")
                     ->select();
        $count = $Model->where( array("is_del"=>'1',"user_id"=>$user_id) )->count();// 查询满足要求的总记录数
        $pager = new \Think\Page($count,$size);// 实例化分页类 传入总记录数和每页显示的记录数
        $page = $pager->show();//分页显示输出
        $this->page = $page;
        $this->data = $res;
    ...
    public function mycollect(){
        ...
        $size = empty($_REQUEST['size']) ? 10 : $_REQUEST['size'];
        $user_id = session('user.user_id');
        /*分布设置______默认为文章_____*/
        if(is_null($_GET['type']) || $_GET['type'] == 1){
            $Model = M("article_collect a");
            $where = "a.user_id='{$user_id}'";
            $Model->where( $where );
            $Model->field("a.article_id,b.title");
            $Model->join("tp_article b ON a.article_id=b.article_id");
            $Model->order('a.collect_id desc');
        }

        /*tab点击事件______咨询收藏_____*/
        if(!is_null($_GET['type']) && $_GET['type'] == 2){
            $Model = M('consult_like a');
            $where = "a.user_id='{$user_id}'";
            $Model->where( $where );
            $Model->field("a.*,b.tittle,detail,createtime,path");
            $Model->join("tp_consult b ON a.consult_id=b.consult_id");
            $Model->order('a.consult_id desc');
        }           
        $Model->page("$p,$size");
        $data = $Model->select();
        if( $_GET['type'] == 2 ){
        ...
        $pager = new \Think\Page($count,$size);// 实例化分页类 传入总记录数和每页显示的记录数
        $page = $pager->show();//分页显示输出
        $this->type = $_GET['type'] ? $_GET['type'] : 1; 
        $this->page = $page;
        $this->data = $data;
        $this->display(); 
    }
    ...
    public function mycomment(){
        ...
        if(is_null($_GET['type']) || $_GET['type'] == '1'){
            ...
            $Model->where( $where );
            $Model->field("a.consult_id,a.tittle,a.detail,b.createtime,b.comment,c.nickname,c.head_pic");
        ...
        if(!is_null($_GET['type']) && $_GET['type'] == '2'){
            ...
            $where = "a.user_id='{$user_id}' AND a.is_show='0'";
            $Model->where( $where );
            $Model->field("a.content,a.add_time,a.user_id,b.nickname,b.head_pic,c.title,c.content as detail");
        ...
        if(!is_null($_GET['type']) && $_GET['type'] == '3'){
            $Model = M('activity_comment a');
            $where = "a.user_id='{$user_id}' AND a.is_show='1'";
            $Model->where( $where );
            $Model->field("a.comment_id,a.add_time,a.content,b.nickname,b.head_pic,c.title,c.content as detail");
            $Model->join("tp_users b ON a.user_id=b.user_id");
        ...
        if(is_null($_GET['type']) || $_GET['type'] == 1){
            $count = $Model->where( $where )
                           ->join("tp_consult_comment b ON a.consult_id=b.consult_id")
                           ->count();
        }else{
            $count = $Model->where( $where )->count();// 查询满足要求的总记录数
        }
    ...
    public function mymessages(){
        $p = empty($_REQUEST['p']) ? 1 : $_REQUEST['p'];
        $size = empty($_REQUEST['size']) ? 10 : $_REQUEST['size'];
        $Model = M("UserMessage a");
        $user_id = session('user.user_id');
        $res = $Model->where("a.user_id='{$user_id}' AND a.is_del='10'")
                     ->field("a.*,b.message,FROM_UNIXTIME(b.send_time,'%Y-%m-%d %H:%i') send_time")
                     ->join("tp_message b ON a.message_id=b.message_id")
                     ->order('a.rec_id desc')
                     ->page("$p,$size")
                     ->select();
        $count = $Model->where( array("is_del"=>'10',"user_id"=>$user_id) )->count();// 查询满足要求的总记录数
        $pager = new \Think\Page($count,$size);// 实例化分页类 传入总记录数和每页显示的记录数
        $page = $pager->show();//分页显示输出
    ...
    */
     ...
     * $id 验证码标示
     */
    private function verifyHandle( $code , $id)
    {
        $verify = new Verify();
        if (!$verify->check( $code , $id ? $id : 'user_login')) {
            $this->error("验证码错误");
        }
    }
