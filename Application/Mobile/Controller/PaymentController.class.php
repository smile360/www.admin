...
class PaymentController extends MobileBaseController {
    ...
    public function Pay(){
        ...
        $order_M = M('order');
        $view_data['order'] = $order_M->join('o left join __ORDER_DETAIL__ od on o.oid = od.oid left join __GOODS__ g on od.pid = g.goods_id')
                                      ->where(array('o.oid'=>$oid))
                                      ->field('o.oid,o.order_no,g.goods_name,od.count,od.money')
                                      ->find();
        $tools = new \JsApiPay();
        $openId = $tools->GetOpenid();
        // var_dump($openId);die;
        $input = new \WxPayUnifiedOrder();
        $input->SetBody( $view_data['order']['goods_name'] );
        $input->SetAttach( $view_data['order']['order_no'] ); //返回订单号
        $input->SetOut_trade_no( $view_data['order']['order_no'] );
        $input->SetTotal_fee( $view_data['order']['money']*100 );
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetGoods_tag("test");
        /* */
        $url = "http://".$_SERVER['HTTP_HOST']."/wechat/Pay/Notify/";
    ...
    public function Notify(){
        require_once THINK_PATH."Library/Vendor/Wxpay/PayNotify.class.php";
        /*生成相应的操作类*/
        $call = new \Vendor\Wxpay\PayNotify();
        $call->Handle(false);
    }                
}
