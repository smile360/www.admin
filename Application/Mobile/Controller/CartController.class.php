...
class CartController extends MobileBaseController {
	...
	// 订单满额优惠活动		                
        ...
        $car_price = array(
            ...
            'user_note' => I('user_note'), // 订单优惠活动优惠了多少钱            
        );
        // 提交订单        
    ...
    public function cart4(){
        $order_id = str_replace(".htmls=","",I('order_id'));
        $order = M("Order a")->field("a.*,b.goods_name")
        ...
        $input->SetAttach( $order['order_no'] ); //返回订单号
        $input->SetOut_trade_no( $order['order_id'] );
        // $input->SetTotal_fee( $order['total_amount']*100 );
        $input->SetTotal_fee( 0.01*100 );
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetGoods_tag("FENGZAKKA");
        /* 回调地址*/
        $url = "http://".$_SERVER['HTTP_HOST']."/Mobile/Payment/Notify/";
        $input->SetNotify_url( $url );
        $input->SetTrade_type("JSAPI");
        $input->SetOpenid( $openId );
        $PayData = \WxPayApi::unifiedOrder( $input );
        $jsApiParameters = $tools->GetJsApiParameters( $PayData );
        // 获取签名
