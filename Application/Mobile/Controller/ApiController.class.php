...
class ApiController extends Controller {
    ...
    **/
    public function SendCode( $phone , $type=null ){
        ...
        if( $phone ){ 
            $findUser = $this->DetectionUser( $phone ); //检测号码
            $_user = $this->getUser( $phone , session('user_id'));
            if($type == 'reg' && $findUser){
                ...
                $return['msg'] = '此手机号码未注册';
                $this->ajaxReturn($return); 
            }else if( $type == 'resetphone'  && $_user){
                // p($_user);exit;
                $return['msg'] = '此手机已绑定用户';
                $this->ajaxReturn($return);
            }else if( $type == 'bind' && $findUser ){
                $return['msg'] = '此手机已绑定用户';
                $this->ajaxReturn( $return );
            }
            $data['phone'] = $phone; 
    ...
    **/
    ...
    public function codeVerify(){
        ...
        if(empty( $phone ) || empty( $code )){
            $this->ajaxReturn(array("msg"=>"请输入正确手机号或验证码"));
        }
        // $Model = D('Users');
        //查下是否存在此用户
        $user =  M('users')->where(array("mobile"=>$phone))->count();
        if( $findpwd == 'findpwd'){
            if( !$user ){
               $this->ajaxReturn(array("msg"=>"此手机号码未注册")); 
            }
            session('phone',$phone);
        }else if($findpwd == 'resetphone'){
            if( $user ){
               $this->ajaxReturn(array("msg"=>"此手机已绑定用户"));  
            }
        }
        //校验验证码
        ...
        if( $codeVerify ){
            if ( time() > $codeVerify['disable_time'] ){
                $this->ajaxReturn(array("msg"=>"验证码已失效!"));
            }else if ( $code != $codeVerify['code'] ){
                $this->ajaxReturn(array("msg"=>"验证码错误!"));
            }else{
                if( $findpwd == 'resetphone'){
                   $resetphone = M('users')->where(array("user_id"=>session('user.user_id'),"mobile"=>session('phone')))->setField("mobile",$phone);
                   if( $resetphone ){
                        session('phone',null);
                        $this->ajaxReturn(array("msg"=>"200"));   
                    }else{
                        $this->ajaxReturn(array("msg"=>"绑定手机失败"));
                    }
                }else{
                    $this->ajaxReturn(array("msg"=>"200"));
                }
    ...
    * @param 2 $user_id string;
    * @return $phone string;
    **/
    public function getUser( $phone , $user_id){
        if( empty($phone) || empty($user_id)){
            return false;
        }
        $data = M("user")->where(array("user_id"=>$user_id,"mobile"=>$phone))->find();
        return $data;
    }
    /** 
    * 检测手机号是否已注册
    ...
    **/
    ...
    public function SendMsg( $phone , $code , $SendModel ){
        ...
        if( $SendModel == 'SMS_37040064' ){
            $setSmsParam = "{\"code\":\"{$code}\",\"product\":\"【枫zakka】\"}";
        }else{
            $setSmsParam = "{\"name\":\"【枫zakka】\",\"code\":\"{$code}\"}";
        }
        $c = new \TopClient;
        $c->appkey = Appkey;
