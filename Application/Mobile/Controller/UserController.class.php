...
class UserController extends MobileBaseController
{
    ...
    public function bind_mobile(){
        if(IS_POST){
            ...
            if(empty( $phone ) || empty( $code )){
                $return['msg'] = '请输入正确手机号或验证码';
                $this->ajaxReturn($return); 
            }
            //检测手机号是否用过
            $findMobile = M('users')->where(array("mobile"=>$phone))->count();
            if( $findMobile ){
                $return['msg'] = '此手机号已经使用过';
                $this->ajaxReturn($return);
            }
            //校验验证码
            $codeVerify = M("SendCode")->where( array("phone"=>$phone,"code"=>$code) )->order('code_id desc')->find();
            if( $codeVerify ){
            ...
            //绑定
            $result = M('users')->where(array('user_id'=>$userInfo['user_id']))->setField(array('mobile'=>$phone,'mobile_validated'=>'1'));
            if( $result ){
                $return['msg'] = 200;
                $this->ajaxReturn($return);
            }else{
                $return['msg'] = '绑定手机码失败，请稍候再试~！';
                $this->ajaxReturn($return);
            }
        }else{
            $this->display();
    ...
    **/
    public function accountSet(){
        if(IS_POST){
            ...
            $oldpsd = encrypt(trim(I('post.oldpsd')));
            $newpsd = encrypt(trim(I('post.newpsd')));
            $confirmpsd = encrypt(trim(I('post.confirmpsd')));
            if( $oldpsw == $newpsd || $oldpsw == $confirmpsd){
                exit(json_encode(array("msg"=>"新密码与旧密码一致")));
            }elseif( $newpsd != $confirmpsd){
                exit(json_encode(array("msg"=>"新密码两次输入不一致")));
            }
            //没有设置过密码的授权用户则定义设置密码
            if( !session('user.password') ){
                $setPwd = $Model->where(array('user_id'=>$user_id))->setField(array("password"=>$confirmpsd));
                if( $setPwd ){
                    $this->ajaxReturn(array("code"=>200));
                }else{
                    $this->ajaxReturn(array("msg"=>"修改失败，请稍候再试！")); 
                }
            }
            //检测旧密码是否正确
            $findpwd = $Model->where(array("user_id"=>$user_id,"password"=>$oldpsd))->count();
            if( !$findpwd ){
                $this->ajaxReturn(array("msg"=>"旧密码错误，请重新输入"));
            }else{
                $result = $Model->where(array("user_id"=>$user_id,"password"=>$oldpsd))->setField(array("password"=>$confirmpsd));
                if( $result ){
                    $this->ajaxReturn(array("code"=>200));
                }else{
                    $this->ajaxReturn(array("msg"=>"修改失败，请稍候再试！"));
    ...
    public function info(){
        $Model = M('users');
        if(IS_POST){
            $data['sex'] = $_POST['sex'] == '男'? 1:2;
            $data['nickname'] = $_POST['username'];
            $data['email'] = $_POST['mail'];
            $city = explode(" ",$_POST['addr']);
            $data['province'] = $city[0];
            $data['city'] = $city[1];
            $data['district'] = $city[2];
            $result = $Model->where(array('user_id'=>$this->user_id))->data( $data )->save();
            // $result = $Model->where(array('user_id'=>'2592'))->data( $data )->save();
            if( $result ){
                $this->ajaxReturn(array('code'=>200));
            }else{
            ...
            $user_info = $Model->where(array("user_id"=>$this->user_id))->find();
            $user_info['sex'] = $user_info['sex'] == 0 ? '保密':($user_info['sex'] == 1 ? '男':'女');
    ...
    /**头像**/
    public function u_headimg(){
        $base = trim(I('post.data'));
        $array = explode(',', $base);
        $base64 = base64_decode($array[1]);
        $res = $this->BaseImg ($base64, 'jpg');
        if( $res ){ 
            $this->deleteImg( $this->user_id );
            // $this->deleteImg( '2592' );
            $data['head_pic'] ='/'.$res;
            $result = M("users")->where(array('user_id'=>$this->user_id))->data( $data )->save();
            // $result = M("users")->where(array('user_id'=>'2592'))->data( $data )->save();
            if( $result ){
                $this->ajaxReturn(array('code'=>200,'path'=>$data['head_pic']));
            }else{
                $this->ajaxReturn(array('code'=>400));
            }     
        }else{
    ...
    public function myactivity(){
        ...
        $limit = $page.',5';
        $where = "a.is_del=1 AND a.user_id=".$this->user_id;
        $result = M("ActivityEntry as a")->field("a.is_status,b.activity_id,title,content,activity_logo,b.start_time")
                                         ->where( $where )
                                         ->join("tp_activity as b ON a.activity_id=b.activity_id AND b.is_del=1")
                                         ->order("a.add_time desc")
                                         ->limit( $limit )
                                         ->select();
        foreach ($result as $key => $value) {
            $result[$key]['content'] = mb_substr( htmlspecialchars_decode($value['content']),0,100);
    ...
    **/
    public function consult(){
        ...
        $model['consult_id'] = time();
        $model['tittle'] = $_POST['title'];
        $model['user_id'] = session('user.user_id');
        $model['detail'] = $_POST['question'];
        $model['createtime'] = time();
        $model['path'] = $path;

        if(M('consult')->add($model)){
            // 处理成功后跳转
            M('payTour')
                ->limit(1)
                ->where(array('is_status'=>30,'user_id'=>$model['user_id'],'status'=>10))
                ->save(array('status'=>20,'use_time'=>time()));
    ...
    **/
    public function myconsult(){
        ...
        if(IS_AJAX){
            $start = I('get.start','');
            $status = I('get.type') == '0' ? '1':I('get.type');
            $data = $db->field("consult_id,tittle,status")
                        ->where(array("user_id"=>$user_id,"is_show"=>'0',"status"=>$status))
                        ->order("createtime desc")
                        ->limit("$start,10")
                        ->select();
            if( $data ){
                $this->ajaxReturn(array('code'=>200,'result'=>$data ));
            }else{
                $this->ajaxReturn(array('code'=>400));
            }
        }else{
            $solveData = $db->field("consult_id,tittle,status")
                        ->where(array("user_id"=>$user_id,"is_show"=>'0',"status"=>'1'))
                        ->order("createtime desc")
                        ->limit(10)
                        ->select();
            $unsolveData = $db->field("consult_id,tittle,status")
                              ->where(array("user_id"=>$user_id,"is_show"=>'0',"status"=>'0'))
                              ->order("createtime desc")
                              ->limit(10)
                              ->select();
            $this->solve = $solveData;
            $this->unsolve = $unsolveData;
            $this->display();
        }
    }
    /**
    ...
    **/
    public function mycollect(){
        $ArticleModel = M("ArticleCollect as a");
        $ConsultModel = M("ConsultCollect as a");
        $user_id = session('user.user_id');
        if(IS_AJAX){
            $start = I('get.start','');
            $type = I('get.type','');//0文章1咨询
            if( $type == 1 ){
            	$Model = $ConsultModel;
            	$Model->field("a.consult_id as id,a.tittle as title,FROM_UNIXTIME(b.createtime,'%Y/%m/%d') as time,b.path as img,b.detail as content");
            	$Model->where("a.user_id=$user_id");
            	$Model->join("tp_consult as b ON a.consult_id=b.consult_id");
            	$Model->limit("$start,10");
            	$data = $Model->select();
            	foreach ($data as $key => $value) {
		    		$data[$key]['img'] = explode(',', $value['img']);
		    		$data[$key]['content'] = mb_substr( $value['content'] , 0 , 100);
		    	}
		    	$return['type'] = 'consult';
            }else{
            	$Model = $ArticleModel;
            	$Model->field("a.article_id as id,b.title,FROM_UNIXTIME(b.publish_time,'%Y/%m/%d') as time,b.thumb as img,b.content");
            	$Model->where("a.user_id=$user_id");
            	$Model->join("tp_article as b ON a.article_id=b.article_id");
            	$Model->limit("$start,10");
            	$data = $Model->select();
            	foreach ($data as $key => $value) {
            ...
            $Article = $ArticleModel->field("a.*,b.title,FROM_UNIXTIME(b.publish_time,'%Y/%m/%d') as publish_time,b.thumb,b.content")
                        			->where("a.user_id=$user_id")
                        			->join("tp_article as b ON a.article_id=b.article_id")
                        			->limit(5)
                        			->select();
            $Consult = $ConsultModel->field("a.*,FROM_UNIXTIME(b.createtime,'%Y/%m/%d') as createtime,b.path,b.detail")
            						->where("a.user_id=$user_id")
            						->join("tp_consult as b ON a.consult_id=b.consult_id")
                               		->limit(5)
                               		->select();
            foreach ($Article as $key => $value) {
            	$Article[$key]['content'] = htmlspecialchars_decode( $value['content'] );
	    	}
    ...
    **/
    public function mycomment(){
        ...
        $user_id = session('user.user_id');
        /*分布设置______咨询评论_____*/
        $Model = M('consult_comment a');
        $where = "a.user_id='{$user_id}' AND a.is_show='0'";
        $Model->where( $where );
        $Model->field("b.consult_id id,b.tittle title,b.detail,a.createtime time,a.comment,c.nickname,c.head_pic");
        $Model->join("tp_consult b ON a.consult_id = b.consult_id");
        $Model->join("tp_users c ON a.user_id=c.user_id");
        $Model->order('b.createtime desc');
        $Model->limit("$start,5");
        $data = $Model->select();
        
        /*tab点击事件______文章_____*/
        $Model_1 = M('article_comment a');
        $where_1 = "a.user_id='{$user_id}' AND a.is_show='0'";
        $Model_1->where( $where_1 );
        $Model_1->field("a.content comment,a.add_time time,a.user_id,b.nickname,b.head_pic,c.title,c.content detail");
        $Model_1->join("tp_users b ON a.user_id=b.user_id");
        $Model_1->join("tp_article c ON a.article_id=c.article_id");
        $Model_1->order('a.add_time desc');
        $Model_1->limit("$start,5");
        $data_a = $Model_1->select();
        /*tab点击事件______活动评论_____*/
        $Model_2 = M('activity_comment a');
        $where_2 = "a.user_id='{$user_id}' AND a.is_show='1'";
        $Model_2->where( $where_2 );
        $Model_2->field("a.comment_id id,a.add_time time,a.content comment,b.nickname,b.head_pic,c.title,c.content detail");
        $Model_2->join("tp_users b ON a.user_id=b.user_id");
        $Model_2->join("tp_activity c ON a.activity_id=c.activity_id");
        $Model_2->order('a.add_time desc');       
        $Model_2->limit("$start,5");
        $data_b = $Model_2->select();
        $this->data_a = $this->eachData( $data_a ,'2');
        $this->data_b = $this->eachData( $data_b ,'2');
        $this->data = $this->eachData( $data);
        $this->display();
    }
    function eachData( $data ,$type=null){
        $result = false;
        if( $data ){
            foreach ($data as $key => $value) {
    ...
    public function ajaxComment(){
        ...
        if($_GET['type'] == '0'){
            ...
            $where = "a.user_id='{$user_id}' AND a.is_show='0'";
            $Model->where( $where );
            $Model->field("a.consult_id id,a.tittle title,a.detail,b.createtime time,b.comment,c.nickname,c.head_pic");
            $Model->join("tp_consult_comment b ON a.consult_id=b.consult_id");
            $Model->join("tp_users c ON b.user_id=c.user_id");
            $Model->order('b.createtime desc');
        }
        ...
        if($_GET['type'] == '1'){
            $_type = '2';
            $Model = M('article_comment a');
            $where = "a.user_id='{$user_id}' AND a.is_show='0'";
            $Model->where( $where );
            $Model->field("a.content comment,a.add_time,a.user_id,b.nickname,b.head_pic,c.title,c.content detail");
            $Model->join("tp_users b ON a.user_id=b.user_id");
            $Model->join("tp_article c ON a.article_id=c.article_id");
            $Model->order('a.add_time desc');
        }
        /*tab点击事件______活动评论_____*/
        if($_GET['type'] == '2'){
            $_type = '2';
            $Model = M('activity_comment a');
            $where = "a.user_id='{$user_id}' AND a.is_show='1'";
            $Model->where( $where );
            $Model->field("a.comment_id id,a.add_time time,a.content comment,b.nickname,b.head_pic,c.title,c.content detail");
            $Model->join("tp_users b ON a.user_id=b.user_id");
            $Model->join("tp_activity c ON a.activity_id=c.activity_id");
            $Model->order('a.add_time desc');
        }            
        $Model->limit("$start,5");
        $data = $Model->select();
        if( $data ){
            foreach ($data as $key => $value) {
                $data[$key]['time'] = date('Y-m-d H:i',$value['time']);
                if($_type == '2'){
                    $data[$key]['detail'] = preg_replace ('/\<p\>|\<\/p\>|\<p\>|<br\/>|\<\/p\>/','',mb_substr( htmlspecialchars_decode($value['detail']),0,60)).'...';
                }
            }
            $return['code'] = 200;
            $return['result'] = $data;
    ...
    * @return  bool
    **/
    public function deleteImg( $user_id ){
        if(empty($user_id)){
            return false;
        }
        $info = M('users')->where(array("user_id"=>$user_id))->field("head_pic")->find();
        if($info){
            $is_file = file_exists(($_SERVER['DOCUMENT_ROOT']) . $info['head_pic']);
            // dump($is_files);exit;
            if($is_file){
                $res = @unlink(($_SERVER['DOCUMENT_ROOT']) . $info['head_pic']);
                // if($res){
                return true;
                // }else{
                //     return false;
    ...
    * @return  $url ; 
    **/
    function BaseImg( $base, $suffix){
        $path = __ROOT__."Public/upload/head_pic/".date("Y-m-d")."/";
        $url = $path.date("Ymd").uniqid().".".$suffix;
        if (!is_dir($path))mkdir($path, 0777,true);
        $img = file_put_contents($url,$base);
        $image = new \Think\Image(); 
        $image->open( $url );
        $image->thumb(96, 96 , \Think\Image::IMAGE_THUMB_CENTER)->save( $url ); //按尺寸缩放
        if( $img ){
            return $url;
        }else{
            return false;
    ...
    **/
    public function message(){
        $data['sytmsg'] = M("UserMessage")->where(array('user_id'=>$this->user_id,'status'=>'0'))->count();
        $data['commsg'] = M("ArticleComment")->where(array("use_user_id"=>$this->user_id,'is_read'=>'0'))->count();
        $data['actmsg'] = M("ActivityComment")->where(array("use_user_id"=>$this->user_id,'is_read'=>'0'))->count();
    ...
    public function mymessages(){
        $page = $_GET['start'] ? $_GET['start'] : 0 ;
        $limit = $page.',5';
        $data = M("UserMessage a")->field("a.*,b.message,b.category,FROM_UNIXTIME(b.send_time,'%Y-%m-%d') send_time")
                                 ->join("tp_message b ON a.message_id=b.message_id")
    ...
    /**系统信息---->详情**/
    public function sytDetail(){
        $id = trim(I('get.id'));
        M('UserMessage')->where(array('user_id'=>$this->user_id,'message_id'=>$id))->setField('status','1');
        echo "页面没有设计！";
        // $this->display();
    }
    ...
    public function activityMsg(){
        $page = $_GET['start'] ? $_GET['start'] : 0 ;
        $limit = $page.',5';
        $Model = M('ActivityComment a');
        $Model->where(array("use_user_id"=>$this->user_id))->setField('is_read','1');
        $commentList = $Model->field("comment_id,activity_id,user_id,username,content,FROM_UNIXTIME(add_time,'%Y-%m-%d %H:%i') add_time,use_user_id,is_read,parent_id")
                            ->where("use_user_id=".$this->user_id." AND is_show='1'")
                            ->order("add_time desc")
                            ->limit( $limit )
                            ->select();
        if( $commentList ){
            foreach( $commentList as $key => $value){
                ...
                $result = $Model->field("a.comment_id,a.comment_id,a.user_id,a.content,FROM_UNIXTIME(a.add_time,'%Y-%m-%d %H:%i') add_time,a.ip_address,a.is_show,a.parent_id,b.head_pic,b.nickname as username")
                                               ->where("a.user_id={$value['use_user_id']} AND a.comment_id={$value['parent_id']}")
                                               ->join("tp_users as b ON a.user_id=b.user_id")
                                               ->find();
                                              
                if(  $result ){
                    $commentList[$key]['reply'] = $result;
                }else{
                    $commentList[$key]['reply'] = NULL;  
                }
            }
        }
        if(IS_AJAX){
            if( $commentList ){
                $this->ajaxReturn(array('code'=>200,'result'=>$commentList));
            }else{
                $this->ajaxReturn(array('code'=>400));
            }
        }else{
