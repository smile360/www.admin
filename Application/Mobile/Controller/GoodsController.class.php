...
class GoodsController extends MobileBaseController {
    public function index(){
        $keyword = I('get.search','','strip_tags');
        //轮播
        ...
        $Model = M("Goods");
        $keyword ? $Model->where("goods_name like '%$keyword%'"):'';
        $Model->field("goods_id,goods_sn,goods_name,shop_price,original_img");
        $Model->group('goods_id');
    ...
    public function ajaxGoods(){
        ...
        $Model->group('goods_id');
        $Model->order('sort desc');
        $Model->limit("$start,$num");
        $goodsList = $Model->select();
        if( $goodsList ){
            $this->ajaxReturn(array("code"=>200,"result"=>$goodsList));
        }else{
