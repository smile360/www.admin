...
<div class="subNavBox"> 
    ...
    <?php if(is_array($filter_spec)): foreach($filter_spec as $key=>$spec): ?><li>   
          ...
          <div class="item">
		  	<div class="img-list">
		       <?php if(is_array($spec)): foreach($spec as $k2=>$v2): ?><img class="img src_<?php echo ($k2); ?>" src="<?php echo ($v2[src]); ?>"><?php endforeach; endif; ?>
		  	</div> 
          <?php if(is_array($spec)): foreach($spec as $k2=>$v2): ?><a href="javascript:;" onclick="switch_spec(this);" data-id="<?php echo ($k2); ?>" title="<?php echo ($v2[item]); ?>" <?php if($k2 == 0): ?>class="hover"<?php endif; ?>>
              ...
              <li style=" border-bottom:1px solid #eeeeee">
                <div class="item1"> 
                  ...
                  <button type="button" class="decrease" onClick="goods_cut();">-</button>
                    <input type="text" class="num" id="number" name="goods_num" value="1" min="10" max="1000"/>
...
<div class="footer_nav">
  ...
  <dl style="width:100%;">
    ...
    <dd class="goumai"><a style="display:block;" href="javascript:void(0);"  onclick="AjaxAddCart(<?php echo ($goods["goods_id"]); ?>,1,0);">立即购买</a> </dd>
  </dl>
</div>
<script type="text/javascript">
...
{
  $('.img').css('display','none');
  var _is = $(spec).data('id');
  $('.src_'+_is).css('display','block');
