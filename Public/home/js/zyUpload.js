		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				<div id="status_info" class="info hidden">选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
					...
					html += '		</div>';
					html += '		<div id="uploadInf" class="upload_inf hide"></div>';
					html += '	</div>';
					html += '</form>';
				}else{
		            ...
		            html += '					<div class="upload_btn">开始上传</div>';
		            html += '				</div>';
		            html += '				<div id="status_info" class="info">支持jpg/jpeg/png格式的图片，宽850*高400以上，图片大小5M内</div>';
		            html += '			</div>';
				    ...
				    html += '							<img class="upload_image" src="'+IMG+'" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funSetStatusInfo = function(files){
				...
				if(num==4){
					$(".webuploader_pick").css("display","none");					
				}else{
					$(".webuploader_pick").css("display","block");	
				}				
			};
